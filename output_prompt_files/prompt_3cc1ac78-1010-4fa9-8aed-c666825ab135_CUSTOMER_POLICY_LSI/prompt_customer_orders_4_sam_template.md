...
2) create LSI for phone as sort/range key (sk) with table partition key as hash key. only order_date, sku in LSI. index name to be of the format lsi__phone__index. add phone as sort/range key (sk) to attribute definition. attribute is string