...
class TestDdbTableTwoDAL:
    ...
    with both positive and negative test cases, including 3 GSI queries.
    """
    ...
    def test_put_item_with_all_fields(self, mock_dynamodb_table):
        ...
        dto = DdbTableTwoDto(
            ...
            attr_str_5="str5",
            attr_str_6="str6",
            gsi1pk_attr_str_7="gsi1_test_pk",
            gsi1sk_attr_str_8="gsi1_test_sk",
            gsi2pk_attr_str_9="gsi2_test_pk",
            gsi2sk_attr_str_10="gsi2_test_sk",
            gsi3pk_attr_str_11="gsi3_test_pk"
        )
        ddb_table_two_put_item(dto)
        item = DdbTableTwo.get("test_pk2", "test_sk2")
        assert item.gsi1pk_attr_str_7 == "gsi1_test_pk"
        assert item.gsi3pk_attr_str_11 == "gsi3_test_pk"
    ...
    def test_gsi1_query_no_results(self, mock_dynamodb_table):
        ...
        """
        create_sample_items()
        results = gsi__gsi1pk_attr_str_7__gsi1sk_attr_str_8__index_query("nonexistent_gsi1")
        assert len(results) == 0
    ...
    def test_gsi2_query_with_sort_condition(self, mock_dynamodb_table):
        """
        Test GSI2 query with gsi2sk_attr_str_10 condition.
        
        Verifies that GSI2 query with sort key condition filters correctly.
        """
        create_sample_items()
        from pynamodb.expressions.operand import Path
        results = gsi__gsi2pk_attr_str_9__gsi2sk_attr_str_10__index_query(
            "gsi2_pk1",
            Path('gsi2sk_attr_str_10') == "gsi2_sk1"
        )
        assert len(results) == 1
        assert results[0].gsi2sk_attr_str_10 == "gsi2_sk1"

    def test_gsi2_query_with_sort_condition_range(self, mock_dynamodb_table):
        ...
        results = gsi__gsi2pk_attr_str_9__gsi2sk_attr_str_10__index_query(
            "gsi2_pk1",
            sort_key_condition=Path('gsi2sk_attr_str_10').between('gsi2_sk1', 'gsi2_sk3')
        )
        
        assert all('gsi2_sk1' <= dto.gsi2sk_attr_str_10 <= 'gsi2_sk3' for dto in results if dto.gsi2sk_attr_str_10 is not None)
    ...
    def test_gsi2_query_no_results(self, mock_dynamodb_table):
        ...
        """
        create_sample_items()
        results = gsi__gsi2pk_attr_str_9__gsi2sk_attr_str_10__index_query("nonexistent_gsi2")
        assert len(results) == 0

    # GSI 3 Tests (hash only, no range key)
    def test_gsi3_query_success(self, mock_dynamodb_table):
        """
        Test GSI3 query by gsi3pk_attr_str_11 (hash only).
    ...
    def test_gsi3_query_with_filter(self, mock_dynamodb_table):
        """
        Test GSI3 query with filter condition.
        
        Verifies that GSI3 query with filter condition works correctly.
        """
        create_sample_items()
        from pynamodb.expressions.operand import Path
        results = gsi__gsi3pk_attr_str_11__index_query(
            "gsi3_pk1",
            filter_key_condition=Path('attr_str_3') == "str3_val1"
        )
        assert len(results) == 1
    ...
    def test_gsi3_query_no_results(self, mock_dynamodb_table):
        ...
        """
        create_sample_items()
        results = gsi__gsi3pk_attr_str_11__index_query("nonexistent_gsi3")
        assert len(results) == 0

    def test_gsi3_query_empty_result(self, mock_dynamodb_table):
        """Test GSI3 query with non-existent key."""
        results = gsi__gsi3pk_attr_str_11__index_query("nonexistent_gsi3")
        
        assert len(results) == 0

    def test_gsi3_query_single_item(self, mock_dynamodb_table):
        """
        Test GSI3 query returning single item.
        
        Verifies that GSI3 query returns correct single item.
        """
        create_sample_items()
        results = gsi__gsi3pk_attr_str_11__index_query("gsi3_pk2")
        assert len(results) == 1
        assert results[0].gsi3pk_attr_str_11 == "gsi3_pk2"
