...
def mock_dynamodb_table(aws_credentials):
    """Create a mock DynamoDB table with GSI and LSI for testing."""
    with mock_aws():
...
def test_lsi_query(mock_dynamodb_table):
    """Test querying the LSI by phone."""
    create_sample_items()
    result = lsi__phone__index_query("cust1")
    assert result is not None
    assert "items" in result
    assert len(result["items"]) == 2
    assert all(dto.pk == "cust1" for dto in result["items"])
...
def test_lsi_query_with_sort_condition(mock_dynamodb_table):
    ...
    create_sample_items()
    result = lsi__phone__index_query(
        "cust1", sort_key_condition=Key("phone").eq("555-1234")
    )
    assert len(result["items"]) == 1
    assert result["items"][0].phone == "555-1234"
...
def test_lsi_query_with_sort_range(mock_dynamodb_table):
    ...
    result = lsi__phone__index_query(
        "cust1",
        sort_key_condition=Key("phone").between("555-0000", "555-9999")
    )
    assert len(result["items"]) == 2
...
def test_lsi_query_empty_result(mock_dynamodb_table):
    ...
    result = lsi__phone__index_query("nonexistent_pk")
    assert len(result["items"]) == 0


def test_lsi_query_nonexistent_phone(mock_dynamodb_table):
    """Test LSI query with non-existent phone value."""
    create_sample_items()
    result = lsi__phone__index_query(
        "cust1", sort_key_condition=Key("phone").eq("000-0000")
    )
    assert len(result["items"]) == 0
