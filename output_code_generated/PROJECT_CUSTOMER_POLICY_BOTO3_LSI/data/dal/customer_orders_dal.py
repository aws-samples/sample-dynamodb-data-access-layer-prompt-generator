...
def customer_orders_query(hash_key: str, range_key_and_condition=None, filter_key_condition=None,
    ...
    try:
        ...
        if index_name:
            ...
            elif index_name == "lsi__phone__index":
...
def lsi__phone__index_query(pk: str, sort_key_condition=None,
                            filter_key_condition=None,
                            last_evaluated_key: dict = None) -> Dict[str, Any]:
    """
    Wrapper for LSI query using lsi__phone__index. Uses the table's
    partition key (pk) as hash key and phone as sort key.

    Args:
        pk (str): Table's partition/hash key
        sort_key_condition: Optional phone sort key condition
        filter_key_condition: Optional filter key condition
        last_evaluated_key (dict): Optional last evaluated key for pagination
    ...
    try:
        ...
        logger.error("Error querying LSI lsi__phone__index for pk=%s: %s", pk, e)
        raise
