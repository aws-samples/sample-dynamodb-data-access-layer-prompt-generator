...
def policy_manager_put_item(dto: PolicyManagerDto) -> None:
    """
    Put an item into the PolicyManager table. Sets created_at, version=1, and TTL.
...
def policy_manager_update_item(dto: PolicyManagerDto) -> None:
    """
    Update an item in the PolicyManager table using optimistic locking
    ...
    try:
        ...
        if dto.policy_id is not None:
            update_parts.append("#policy_id = :policy_id")
            expr_values[':policy_id'] = dto.policy_id
            expr_names['#policy_id'] = 'policy_id'
        if dto.policy_name is not None:
            update_parts.append("#policy_name = :policy_name")
            expr_values[':policy_name'] = dto.policy_name
            expr_names['#policy_name'] = 'policy_name'
        if dto.risk_id is not None:
            update_parts.append("#risk_id = :risk_id")
            expr_values[':risk_id'] = dto.risk_id
            expr_names['#risk_id'] = 'risk_id'
        if dto.address is not None:
            update_parts.append("#address = :address")
            expr_values[':address'] = dto.address
            expr_names['#address'] = 'address'
        if dto.premium is not None:
            update_parts.append("#premium = :premium")
            expr_values[':premium'] = Decimal(str(dto.premium))
            expr_names['#premium'] = 'premium'
        if dto.sum_insured is not None:
            update_parts.append("#sum_insured = :sum_insured")
            expr_values[':sum_insured'] = Decimal(str(dto.sum_insured))
            expr_names['#sum_insured'] = 'sum_insured'
        if dto.policy_version is not None:
            update_parts.append("#policy_version = :policy_version")
            expr_values[':policy_version'] = dto.policy_version
            expr_names['#policy_version'] = 'policy_version'
        if dto.start_date is not None:
            update_parts.append("#start_date = :start_date")
            expr_values[':start_date'] = dto.start_date
            expr_names['#start_date'] = 'start_date'
        if dto.end_date is not None:
            update_parts.append("#end_date = :end_date")
            expr_values[':end_date'] = dto.end_date
            expr_names['#end_date'] = 'end_date'
        if dto.limit is not None:
            update_parts.append("#attr_limit = :attr_limit")
            expr_values[':attr_limit'] = Decimal(str(dto.limit))
            expr_names['#attr_limit'] = 'limit'
        if dto.deductible is not None:
            update_parts.append("#deductible = :deductible")
            expr_values[':deductible'] = dto.deductible
            expr_names['#deductible'] = 'deductible'
        if dto.amount is not None:
            update_parts.append("#amount = :amount")
            expr_values[':amount'] = Decimal(str(dto.amount))
        ...
        if dto.gsipk2 is not None:
            update_parts.append("#gsipk2 = :gsipk2")
            expr_values[':gsipk2'] = dto.gsipk2
            expr_names['#gsipk2'] = 'gsipk2'
        if dto.gsisk2 is not None:
            update_parts.append("#gsisk2 = :gsisk2")
            expr_values[':gsisk2'] = dto.gsisk2
            expr_names['#gsisk2'] = 'gsisk2'
...
def policy_manager_get_item(pk: str, sk: str) -> Optional[PolicyManagerDto]:
    """
    Get an item from the PolicyManager table.
...
def policy_manager_delete_item(pk: str, sk: str) -> None:
    """
    Delete an item from the PolicyManager table using optimistic locking.
...
def policy_manager_query(hash_key: str, range_key_and_condition=None, filter_key_condition=None,
                         ...
                         last_evaluated_key: dict = None) -> List[PolicyManagerDto]:
    """
    Query the PolicyManager table using specified parameters until all pages
    ...
    try:
        ...
        if index_name:
            ...
            elif index_name == "lsi__policy_version__index":
...
def lsi__policy_version__index_query(pk: str, sort_key_condition=None,
                                     filter_key_condition=None,
                                     last_evaluated_key: dict = None) -> Dict[str, Any]:
    """
    Wrapper for LSI query using lsi__policy_version__index. Uses the table's
    partition key (pk) as hash key and policy_version as sort key.

    Args:
        pk (str): Table's partition/hash key
    ...
    try:
        ...
        logger.error("Error querying LSI lsi__policy_version__index for pk=%s: %s", pk, e)
        raise
