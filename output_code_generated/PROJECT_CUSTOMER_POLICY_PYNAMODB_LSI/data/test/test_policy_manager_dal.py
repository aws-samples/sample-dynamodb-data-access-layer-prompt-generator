...
class TestPolicyManagerDAL:
    ...
    with both positive and negative test cases, including GSI1, GSI2, and LSI queries.
    """
    ...
    def test_put_item_sets_version_and_created_at(self, mock_dynamodb_table):
        ...
        policy_manager_put_item(dto)
        item = PolicyManager.get("version_pk", "version_sk")
    ...
    def test_update_item_increments_version(self, mock_dynamodb_table):
        ...
        """
        create_sample_items()
        item_before = PolicyManager.get("policy1", "version1")
        ...
        policy_manager_update_item(dto)
        item_after = PolicyManager.get("policy1", "version1")
    ...
    def test_get_item_nonexistent_pk(self, mock_dynamodb_table):
        ...
        """
        create_sample_items()
        dto = policy_manager_get_item("no_such_pk", "version1")
        assert dto is None
    ...
    def test_get_item_nonexistent_sk(self, mock_dynamodb_table):
        ...
        """
        create_sample_items()
        dto = policy_manager_get_item("policy1", "no_such_sk")
        assert dto is None
    ...
    def test_lsi_query_success(self, mock_dynamodb_table):
        """
        Test LSI query by pk and policy_version.
        
        Verifies that LSI query returns items matching pk with policy_version as sort key.
        """
        create_sample_items()
        results = lsi__policy_version__index_query("policy1")
        assert len(results) == 2
        assert all(dto.pk == "policy1" for dto in results)

    def test_lsi_query_with_sort_condition(self, mock_dynamodb_table):
        """
        Test LSI query with policy_version sort key condition.
        
        Verifies that LSI query with sort key condition filters correctly.
        """
        create_sample_items()
        from pynamodb.expressions.operand import Path
        results = lsi__policy_version__index_query(
            "policy1",
            sort_key_condition=Path('policy_version') == "v1"
        )
        assert len(results) == 1
        assert results[0].policy_version == "v1"

    def test_lsi_query_with_filter(self, mock_dynamodb_table):
        """
        Test LSI query with filter condition.
        
        Verifies that LSI query with filter condition works correctly.
        """
        create_sample_items()
        from pynamodb.expressions.operand import Path
        results = lsi__policy_version__index_query(
            "policy1",
            filter_key_condition=Path('policy_name') == "Home Insurance"
        )
        assert len(results) == 2
        assert all(dto.policy_name == "Home Insurance" for dto in results)

    def test_lsi_query_with_sort_range(self, mock_dynamodb_table):
        """
        Test LSI query with policy_version range condition.
        
        Verifies that LSI query with range condition on policy_version works.
        """
        create_sample_items()
        from pynamodb.expressions.operand import Path
        results = lsi__policy_version__index_query(
            "policy1",
            sort_key_condition=Path('policy_version').between('v1', 'v3')
        )
        assert len(results) == 2

    def test_lsi_query_empty_result(self, mock_dynamodb_table):
        """
        Test LSI query with no matching results.
        
        Verifies that empty list is returned when no items match LSI query.
        """
        create_sample_items()
        results = lsi__policy_version__index_query("nonexistent_pk")
        assert len(results) == 0

    def test_lsi_query_nonexistent_version(self, mock_dynamodb_table):
        """
        Test LSI query with nonexistent policy_version value.
        
        Verifies that LSI query returns empty when policy_version doesn't match.
        """
        create_sample_items()
        from pynamodb.expressions.operand import Path
        results = lsi__policy_version__index_query(
            "policy1",
            sort_key_condition=Path('policy_version') == "v99"
        )
        assert len(results) == 0
    ...
    def test_model_ttl_for_non_test_object(self, mock_dynamodb_table):
        ...
        """
        import os
        os.environ['POLICY_MANAGER_TTL_DAYS'] = '0'
    ...
    def test_gsi1_query_empty_result(self, mock_dynamodb_table):
        """Test GSI1 query with non-existent key."""
        results = gsi__gsipk1__gsisk1__index_query("nonexistent_gsi")
        
        assert len(results) == 0

    def test_gsi2_query_empty_result(self, mock_dynamodb_table):
