...
class TestCustomerOrdersDAL:
    ...
    with both positive and negative test cases, including GSI and LSI queries.
    """
    ...
    def test_lsi_query_success(self, mock_dynamodb_table):
        """
        Test LSI query by pk and phone.
        
        Verifies that LSI query returns items matching pk with phone as sort key.
        """
        create_sample_items()
        results = lsi__phone__index_query("cust1")
        assert len(results) == 2
        assert all(dto.pk == "cust1" for dto in results)

    def test_lsi_query_with_sort_condition(self, mock_dynamodb_table):
        """
        Test LSI query with phone sort key condition.
        
        Verifies that LSI query with sort key condition filters correctly.
        """
        create_sample_items()
        from pynamodb.expressions.operand import Path
        results = lsi__phone__index_query(
            "cust1",
            sort_key_condition=Path('phone') == "555-1234"
        )
        assert len(results) == 1
    ...
    def test_lsi_query_with_filter(self, mock_dynamodb_table):
        ...
        results = lsi__phone__index_query(
            "cust1",
            filter_key_condition=Path('order_date') == "2024-01-15"
        )
        assert len(results) == 1
        assert results[0].order_date == "2024-01-15"

    def test_lsi_query_with_sort_range(self, mock_dynamodb_table):
        """
        Test LSI query with phone range condition.
        
        Verifies that LSI query with range condition on phone works.
        """
        create_sample_items()
        from pynamodb.expressions.operand import Path
        results = lsi__phone__index_query(
            "cust1",
            sort_key_condition=Path('phone').between('555-0000', '555-9999')
        )
        assert len(results) == 2
    ...
    def test_lsi_query_empty_result(self, mock_dynamodb_table):
        ...
        """
        create_sample_items()
        results = lsi__phone__index_query("nonexistent_pk")
        assert len(results) == 0

    def test_lsi_query_nonexistent_phone(self, mock_dynamodb_table):
        """
        Test LSI query with nonexistent phone value.
        
        Verifies that LSI query returns empty when phone doesn't match.
        """
        create_sample_items()
        from pynamodb.expressions.operand import Path
        results = lsi__phone__index_query(
            "cust1",
            sort_key_condition=Path('phone') == "000-0000"
        )
        assert len(results) == 0
