...
class CustomerOrders(Model):
    ...
    Attributes:
        ...
        lsi_phone_index: LSI for querying by pk and phone
    """
    ...
    # LSI
    class LsiPhoneIndex(LocalSecondaryIndex):
        """
        LSI for pk (hash) and phone (range).
        Includes order_date, sku.
        """
        class Meta:
            index_name = "lsi__phone__index"
            projection = IncludeProjection(non_attr_keys=['order_date', 'sku'])

        pk = UnicodeAttribute(hash_key=True)
        phone = UnicodeAttribute(range_key=True)
    
    lsi_phone_index = LsiPhoneIndex()
